...
const processPdf = async (pdfpath = "pdfs/Goi_Mobi_1.pdf") => {
...
const init = async () => {
  ...
  const query = "Thông tin gói D10?"; // it should match processed PDFs
  const relevantChunksMatchingQuery = await retrieveRelevantChunks(query);
  console.log(
    `Chunks matching for query "${query}"`,
    relevantChunksMatchingQuery
  );
  if (!relevantChunksMatchingQuery.length) {
    console.log(
      "No matching chunks found, Need to parse PDF and store embeddings"
    );
